...
export default function WeeklyWorkout({
    ...
    const formatDetail = ({ sets, reps, weight }) =>
        [
            sets && `${sets} sets`,
            reps && `x ${reps} reps`,
            weight?.value && `${weight.value} ${weight.unit}`,
        ]
            .filter(Boolean)
            .join(', ');
    ...
    const getSearchUrl = name =>
        `https://www.google.com/search?q=${encodeURIComponent(`how to do ${name}`)}`;
            ...
            prev.map(dayObj => {
                ...
                return {
                    ...
                    workouts: dayObj.workouts.map(w =>
                        w.workout_id === updatedWorkout.workout_id
                            ? updatedWorkout
                            : w
                    ),
                };
    ...
    return (
        <div
            className='accordion workout-container'
            id='weeklyWorkoutAccordion'
        >
            {loadingWorkout && (
                <div className='workout-loading-overlay'>
                    <div
                        ...
                        role='status'
                    />
                    <span className='loading-text ms-2'>
                    ...
                    return (
                        <div key={day} className='accordion-item'>
                            ...
                            >
                                ...
                                >
                                    <span className='accordion-button-label'>
                                        {day}
                                    </span>
                                    <span
                                        className={`category-badge ${CATEGORY_CLASSES[category] || 'category-badge--default'}`}
                                    >
                                        {category}
                                    </span>
                                </button>
